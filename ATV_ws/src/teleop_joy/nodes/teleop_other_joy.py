...
# same file as "teleop_other_joy_sim_only.py", but the emergency stop is not taken into accout
...
def run_control(button_keys, analog_keys):
    global LEFT, RIGHT, speed, movement_msg, stepper_is_disabled, forward_reverse, disabled_stepper_pub
    ...
    if (not stepper_is_disabled): #allow teleoperation only if the stepper motor is engaged
        if RIGHT:
...
def main():
    global movement_msg, disabled_stepper_pub
